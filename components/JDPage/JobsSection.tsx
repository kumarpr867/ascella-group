      ...
      <motion.div layout>
          ...
          <Reveal variants={slideInFromLeft(0.2)} className="flex items-center justify-between w-full mx-20">
            {companies.map((company) => (
              <button
                ...
                className={`relative whitespace-nowrap text-sm sm:text-base pb-2 transition ${
                  activeCompany === company ? "text-white" : "text-white/50 hover:text-white"
                }`}
              >
                {company}
        ...
        <div className="flex lg:hidden items-center border-b border-color">
          <button
            ...
            className={`relative shrink-0 whitespace-nowrap text-sm pb-3 pt-2 pl-6 pr-3 transition ${
              activeCompany === "All roles" ? "text-white" : "text-white/50"
            }`}
          >
            All roles
            {activeCompany === "All roles" && (
            ...
            {scrollableCompanies.map((company) => (
              <button
                ...
                className={`relative shrink-0 whitespace-nowrap text-sm transition ${
                  activeCompany === company ? "text-white" : "text-white/50"
                }`}
              >
                {company}
                {activeCompany === company && (
                  ...
                  <div className="flex flex-wrap gap-3">
                    {statuses.map((status) => (
                      <button
                        ...
                        className={`px-4 py-1 rounded-xl text-[12px] transition ${
                          activeStatus === status
                            ? "bg-white text-black"
                        ...
                        */}
                        ...
                        >
                          <Image
                            ...
                            className={`border border-color object-cover pointer-events-none ${
                              job.status === "Open position" ? "opacity-50" : "opacity-20"
                            }`}
                          />
                        ...
                        <div className="flex flex-col md:hidden flex-1">
                          ...
                          <h3
                            className={`text-[13px] font-medium leading-tight line-clamp-2 mb-1 ${
                              job.status === "Open position" ? "text-white" : "text-gray-400"
                            }`}
                          >
                            {job.title}
                          </h3>
                        ...
                        <div className="hidden md:flex flex-col flex-1">
                          ...
                          <div className="flex justify-start items-center gap-3 mb-6">
                            <span
                              className={`text-b3 px-4 py-1 rounded-2xl ${
                                job.status === "Open position"
