...
const navLinks = [
  ...
  },
  {
    label: "Careers",
    href: "/careers",
    // mobileOnly: true → desktop renders as plain link, mobile/tablet renders as accordion dropdown
    mobileOnly: true,
    children: [
          ...
          <div className="flex justify-between items-center h-full">
            ...
            <nav className="hidden lg:flex items-center font-medium relative">
              {navLinks.map((link, index) => {
                ...
                // mobileOnly items have children only for mobile — on desktop treat as plain link
                const hasDesktopDropdown = !!link.children && !link.mobileOnly;
                ...
                return (
                  <div
                    ...
                    onMouseEnter={() => {
                      if (hasDesktopDropdown) setOpenDesktopDropdown(link.label);
                    }}
                    onMouseLeave={() => {
                      if (hasDesktopDropdown) setOpenDesktopDropdown(null);
                    }}
                  >
                    <button
                    ...
                    {/* Desktop dropdown — only for non-mobileOnly items */}
                    <AnimatePresence>
                      {hasDesktopDropdown && isOpen && (
                        <motion.div
        ...
        <nav className="flex flex-col flex-1 px-7 pt-2 overflow-y-auto">
          {navLinks.map((link) => {
            ...
            return (
              <div
                key={link.label}
                className={`border-b border-color transition-colors ${isActive ? "bg-white/10" : ""}`}
              >
                {/* Plain link: has href but no children */}
                {link.href && !link.children ? (
                  ...
                  /* Accordion dropdown: items with children (including mobileOnly like Careers) */
                  <div className="py-4">
                    <button
