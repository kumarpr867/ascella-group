        ...
        >
          {typeof image === "string" ? (
            <Image
              ...
              width={550}
              height={550}
              className="object-contain w-[180px] h-[180px] sm:w-[280px] sm:h-[280px] md:w-[380px] md:h-[380px] lg:w-[480px] lg:h-[480px] xl:w-[550px] xl:h-[550px]"
            />
          ) : (
            image // Render the SVG component
          )}
        </motion.div>
      </motion.div>
      ...
      <div className="mx-auto w-full relative">
        ...
        <div className="hidden lg:grid lg:grid-cols-[10%_25%_55%_10%] relative">
          ...
          <aside className="relative z-10 p-12">
            <div className="sticky top-24 flex flex-col justify-between h-[80vh]">
              <Heading text="OWNERSHIP" />
              <ul className="mt-20 space-y-6">
                {sections.map((item, idx) => (
                  <li
                    key={idx}
                    onClick={() => scrollToSection(idx)}
                    className={`text-sm uppercase tracking-widest transition-all duration-300 cursor-pointer ${activeIndex === idx
              ...
              <ul className="mt-10 space-y-4">
                {sections.map((item, idx) => (
                  <li
                    key={idx}
                    onClick={() => scrollToSection(idx)}
                    className={`text-xs uppercase tracking-widest transition-all duration-300 cursor-pointer ${activeIndex === idx
                      ? "text-white font-medium"
                      : "text-white/40 hover:text-white/70"
                      }`}
                  >
