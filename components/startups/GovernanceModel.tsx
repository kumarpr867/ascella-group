    ...
    <>
      ...
      <div className="gov-wrapper bg-black text-white min-h-screen w-full font-sans overflow-hidden relative">
        ...
        {/* ══════════════════════════════════════
            ...
            - Two vertical lines with left/right margin
            - Circle centered between the two vertical lines
            - GOVERNANCE MODEL label + heading overlaid on circle
            - Horizontal line after circle ends
            - Gap + horizontal line
            - Operating Framework content
            - Horizontal line
            - Progressive Introduction content
            - Horizontal line
            - Execution Remains content
            - Bottom horizontal line
        ══════════════════════════════════════ */}
        <div className="gov-mobile flex-col w-full relative">

          {/* Vertical guide lines (full height of mobile section) */}
          {/* These are rendered as absolute lines behind content */}
          <div className="absolute inset-0 pointer-events-none z-0" aria-hidden="true">
            {/* Left vertical line — 12% from left */}
            <div className="absolute top-0 bottom-0 border-l border-gray-400" style={{ left: '12%' }}></div>
            {/* Right vertical line — 12% from right */}
            <div className="absolute top-0 bottom-0 border-r border-gray-400" style={{ right: '12%' }}></div>
          </div>

          {/* TOP edge-to-edge horizontal line */}
          <div className="w-full border-t border-gray-400 relative z-10"></div>

          {/* ── CIRCLE SECTION ── */}
          {/* Height = diameter of circle so full circle fits */}
          <div className="relative z-10 w-full overflow-hidden" style={{ height: '76vw' }}>

            {/* 
              Circle sits between vertical lines: left=12%, right=12%, so width=76vw
              It's a perfect circle: width=76vw, height=76vw
              Centered vertically so the horizontal grid line (at 50% of circle height = 38vw from top) 
              aligns with the horizontal border between Row1 and Row2 on desktop feel
            */}
            <div
              ...
              style={{
                left: '12%',
                width: '76%',
                paddingBottom: '76%', /* makes height = width = perfect circle */
                top: '0px',
                borderRadius: '9999px',
            ...
            {/* 
              HORIZONTAL divider line through circle center (at 50% height = 38vw from top)
              Spans only between the two vertical lines (12% to 88%)
            */}
            <div
              className="absolute z-10"
              style={{
                top: '38vw', /* 50% of circle height (76vw / 2) */
                left: '12%',
                width: '76%',
                height: '1px',
            ...
            {/* 
              VERTICAL divider line through circle center (at 50% width = 50% of page)
              Only within circle height (0 to 76vw)
            */}
            <div
              className="absolute z-10"
              style={{
            ...
            {/* Governance Model label + heading centered on circle */}
            <div
          ...
          {/* Horizontal line after circle (between vertical lines) */}
          ...
          {/* Horizontal line before Operating Framework */}
          <div className="relative z-10 w-full" style={{ paddingLeft: '12%', paddingRight: '12%' }}>
            <div className="border-t border-gray-400 w-full"></div>
          </div>
          ...
          {/* BOTTOM edge-to-edge horizontal line */}
          <div className="relative z-10 w-full" style={{ paddingLeft: '12%', paddingRight: '12%' }}>
            <div className="border-t border-gray-400 w-full"></div>
          </div>
